...
import { assertMobileCompatibleJavaScript, assertReadableJavaScript } from './build-output.ts';
...
await wrapCliTask(async () => {
  await build({ customizeEsbuildOptions: customizeBuildOptions });
...
async function finalizeBuildArtifacts(): Promise<void> {
  ...
  assertMobileCompatibleJavaScript(mainJavaScript);
